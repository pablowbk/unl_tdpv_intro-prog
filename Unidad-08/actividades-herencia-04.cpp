...
protected:
  ...
  clock_t
      tempo; // variable para ajustar la velocidad de movimiento de la pelotita
  clock_t
      paso; // variable para ajustar la velocidad de movimiento de la pelotita
  int velocidad;
...
Auto1::Auto1(int vel, int x0, int y0) {
  ...
  velocidad = vel;
  paso = CLOCKS_PER_SEC / velocidad; // cada cuanto se permite mover la pelotita
...
void Auto2::update() {
  ...
  if (tempo + paso < clock()) {
    dibujar();
    tempo = clock(); // tomamos el tiempo para saber la proxima vez que movemos
                     // la pelotita
  }
}
...
int Juego::checkCollision() {
  ...
  if (x1 > (x2 - ancho) && x1 < (x2 + ancho) && y1 > (y2 - alto) &&
      y1 < (y2 + alto)) {
    return 2;
  }

  if (x1 > (x3 - ancho) && x1 < (x3 + ancho) && y1 > (y3 - alto) &&
      y1 < (y3 + alto)) {
    return 3;
  }
